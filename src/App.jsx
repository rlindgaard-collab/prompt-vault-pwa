...
export default function App() {
  const [csvUrl, setCsvUrl] = useState(() => localStorage.getItem("pv_csv_url") || "https://docs.google.com/spreadsheets/d/1AyGePLHz7-W8uETCoV-cGVvcQRcv-2o3o0p30KZVCS4/export?format=csv&gid=0");
  ...
  useEffect(() => {
    localStorage.setItem("pv_csv_url", csvUrl);
  }, [csvUrl]);

  useEffect(() => {
    localStorage.setItem("pv_mapping", JSON.stringify(mapping));
  }, [mapping]);
  ...
  const allTags = useMemo(() => {
    const set = new Set();
    parsed.forEach(p => p.tags.split(",").map(t => t.trim()).filter(Boolean).forEach(t => set.add(t)));
  ...
  async function shareText(txt, title) {
    try {
      if (navigator.share) {
        await navigator.share({ title: title || "Prompt", text: txt });
      } else {
        copyText(txt);
        alert("Din enhed understøtter ikke deling herfra – teksten er kopieret i stedet.");
      }
    } catch {}
  }
    ...
    <div className="min-h-screen bg-white text-slate-900">
      <div className="max-w-xl mx-auto p-4 sm:p-6">
        <header className="mb-4 flex items-center justify-between">
          ...
          <button
            onClick={openChatGPT}
            className="inline-flex items-center gap-2 px-3 py-2 rounded-xl bg-slate-900 text-white active:scale-[0.98]"
          >
                    ...
                    <header className="mb-1 flex items-center justify-between gap-2">
                      ...
                      <div className="flex items-center gap-1">
                        <button
                          ...
                          className="px-2 py-1.5 rounded-lg border border-slate-200 text-sm hover:bg-slate-50"
                          title="Kopiér"
                        >
                          <Clipboard className="w-4 h-4" />
                        </button>
                        <button
                          onClick={() => shareText(p.prompt, p.title)}
                          className="px-2 py-1.5 rounded-lg border border-slate-200 text-sm hover:bg-slate-50"
                          title="Del"
                        >
...
function guessHeader(headers, candidates) {
  ...
  for (const c of candidates) {
    const found = lower.find(h => h.low === c);
    if (found) return found.raw;
  }
  for (const c of candidates) {
    const found = lower.find(h => h.low.startsWith(c));
    if (found) return found.raw;
  }
  return "";
}
